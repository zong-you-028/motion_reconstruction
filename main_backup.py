...
def interactive_mode():
    ...
    while True:
        ...
        if choice == '0':
            ...
            data_dir = input(r"數據目錄 [D:\rppg\motion_reconstruction\rppg_split\train]: ").strip() or r"D:\rppg\motion_reconstruction\rppg_split\train"
            ...
            data_dir = input(r"數據目錄 [D:\rppg\motion_reconstruction\rppg_split\test]: ").strip() or r"D:\rppg\motion_reconstruction\rppg_split\test"
            ...
            # 步驟 2: 診斷數據質量
            print("\n[步驟 2/4] 診斷數據質量")
            diagnose_all_subjects(local_output)
            
            # 步驟 3: 訓練模型
            print("\n[步驟 3/4] 訓練模型")
            split_dir = input(r"數據分割目錄 [D:\rppg\motion_reconstruction\rppg_split]: ").strip() or r"D:\rppg\motion_reconstruction\rppg_split"
            train_dir = str(Path(split_dir) / "train")
            output_dir = input(r"輸出目錄 [D:\rppg\motion_reconstruction\projection_models]: ").strip() or r"D:\rppg\motion_reconstruction\projection_models"
            epochs = int(input("訓練輪數 [50]: ").strip() or 50)
            model_path = train_projection_model(train_dir, output_dir, epochs=epochs)
            
            # 步驟 4: 評估模型
            if model_path:
                print("\n[步驟 4/4] 評估模型")
                test_dir = str(Path(split_dir) / "test")
                eval_output_dir = input(r"評估結果目錄 [.\evaluation_results]: ").strip() or r".\evaluation_results"
                evaluate_model(model_path, test_dir, eval_output_dir)
                
                print("\n" + "="*80)
                print("✅ 完整流程完成！")
                print("="*80)
                print(f"  訓練數據: {train_dir}")
                print(f"  測試數據: {test_dir}")
                print(f"  模型: {model_path}")
                print(f"  評估結果: {eval_output_dir}")
            else:
                print("\n✗ 訓練失敗，未執行評估步驟")
